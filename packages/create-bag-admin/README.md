...
一个面向 `@bag/*` 生态的初始化脚手架，用来快速生成基于 `@bag/host-vue` 的后台宿主项目。
...
```bash
node packages/create-bag-admin/bin/index.mjs demo-admin --bag-version workspace:*
```
...
- `with-demo-plugins`: 教学型模板，预装多插件示例，演示嵌套菜单、插件依赖、`activeMenu`、`noCache` 和 `cacheKey`
