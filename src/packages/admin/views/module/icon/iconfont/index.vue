...
import cgIcons from '@/assets/icon/iconfont/cg-icons';
import rcIcons from '@/assets/icon/iconfont/rc-icons';
import xtIcons from '@/assets/icon/iconfont/xt-icons';
import fsIcons from '@/assets/icon/iconfont/fs-icons';
import tqIcons from '@/assets/icon/iconfont/tq-icons';
import shlIcons from '@/assets/icon/iconfont/shl-icons';
...
export default defineComponent({
    setup() {
        ...
        const handleToClipboard = (str: string) => {
            toClipboard(str).then((res: any) => {
                let html = [];
                html.push(h('i', {class: res.text, style: {marginRight: '10px', color: '#52c41a'}},));
                html.push(h('span', {}, `使用方式：<i class="${res.text}"></i>`));
