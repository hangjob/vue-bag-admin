...
export default defineComponent({
    setup() {
        ...
        const mockColumns = (colSize: number): Promise<any[]> => {
            return new Promise((resolve) => {
                setTimeout(() => {
                    ...
                    const result = XEUtils.clone(
                        columnList.slice(0, colSize),
                        true
                    )
                    resolve(result)
                }, 100)
        ...
        const loadColumnAndData = (colSize: number, rowSize: number) => {
            ...
            Promise.all([mockColumns(colSize), mockList(rowSize)]).then(
                (rest) => {
                    ...
                    if ($grid) {
                        Promise.all([
                            ...
                            VXETable.modal.message({
                                content: `渲染 ${colSize} 列 ${rowSize} 行，用时 ${
                                    Date.now() - startTime
                                }毫秒`,
                                status: 'info',
                            })
