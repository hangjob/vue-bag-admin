<template>
    <n-el tag="div" class="flex flex-shrink-0 border-solid border-b bag-border-color items-center">
        ...
        <n-dropdown key-field="localesKey" :render-label="(item)=>$global?.helpers?.formatTitle($global, item)" :options="popselectOptions" @select="(key)=>handlePopselectOptions($global,$globalStore,key)">
            <n-icon size="22" class="px-2 w-auto flex-shrink-0 cursor-pointer">
...
import { ChevronBackOutline,ChevronForward,HappyOutline,Expand,SyncOutline,PowerOutline,ArrowBackOutline,ArrowForward,
    Resize,LeafOutline,CloseOutline } from "@vicons/ionicons5"
...
const handlePopselectOptions = ($global,$globalStore,key)=>{
    ...
    const title =  $globalStore.currentRouter.meta.title
